...
const FigmaCanvas: React.FC<FigmaCanvasProps> = ({ items, onBubbleClick }) => {
  ...
  const [canvasSize] = useState({ width: 2000, height: 2000 }) // Large canvas size
  ...
  useEffect(() => {
    ...
    const newData: SimulationNode[] = Object.keys(categoryData).map((category) => {
      ...
      return {
        r: Math.max(40, count * 12), // Larger circles for better visibility
    ...
    const circles = mainGroup
      ...
      .attr("stroke", (d) => {
        ...
        if (positiveRatio > 0.6) return "#10b981"
        if (negativeRatio > 0.6) return "#ef4444"
        return "#f59e0b"
      })
    ...
    circles
      ...
      .on("mouseout", function (event, d) {
        d3.select(this).transition().duration(200).attr("stroke-width", 6).attr("r", d.r)
      })
    ...
    const handleResize = () => {
      ...
      simulation.force("center", d3.forceCenter(newCenterX, newCenterY).strength(0.1)).alpha(0.3).restart()
    }
    ...
    >
      ...
      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-lg p-3 shadow-lg">
        <div className="text-xs text-gray-600 space-y-1">
          ...
          <div className="text-xs text-gray-500">• Mouse wheel to zoom • Drag to pan • Drag bubbles to move</div>
        </div>
      </div>
    </div>
  )
